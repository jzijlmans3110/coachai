...
export default function Pipeline() {
  ...
  const columns: { key: Status; labelKey: string; icon: React.ReactNode; color: string; bg: string; border: string }[] = [
    { key: 'intake',    labelKey: 'status_intake',   icon: <UserPlus className="h-4 w-4" />,    color: 'text-blue-600',    bg: 'bg-blue-50',    border: 'border-blue-200' },
    { key: 'actief',    labelKey: 'status_actief',   icon: <Users className="h-4 w-4" />,       color: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-200' },
    { key: 'inactief',  labelKey: 'status_inactief', icon: <PauseCircle className="h-4 w-4" />, color: 'text-amber-600',   bg: 'bg-amber-50',   border: 'border-amber-200' },
    { key: 'gestopt',   labelKey: 'status_gestopt',  icon: <XCircle className="h-4 w-4" />,     color: 'text-slate-400',   bg: 'bg-slate-50',   border: 'border-slate-200' },
  ]
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{t('pipeline_title')}</h1>
          <p className="text-slate-400 text-sm mt-0.5">{t('pipeline_desc')}</p>
        </div>
        {intakeLink && (
          <div className="flex items-center gap-3">
            ...
            <button onClick={copyIntakeLink}
              ...
              <UserPlus className="h-4 w-4" /> {t('copy_intake_link')}
            </button>
          </div>
        )}
      </div>
              ...
              <div className={`flex-1 border-x border-b ${col.border} rounded-b-2xl p-3 space-y-2 min-h-[400px] bg-white`}>
                {colClients.length === 0 && (
                  <div className="flex items-center justify-center h-24">
                    <p className="text-xs text-slate-300">{t('no_clients_col')}</p>
                  </div>
                )}
        ...
        <div className="mt-6 bg-blue-50 border border-blue-100 rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className="font-bold text-blue-700 text-sm">
              {t('intake_waiting', { n: byStatus('intake').length, s: byStatus('intake').length !== 1 ? 's' : '' })}
            </h3>
          </div>
          <p className="text-xs text-blue-600">{t('intake_action')}</p>
        </div>
      )}
    </div>
  )
